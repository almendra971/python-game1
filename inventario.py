...
class Inventory:
    ...
    def _draw_hotbar(self, screen):
        ...
        x = (self.screen_width - hotbar_width) // 2
        y = self.screen_height - self.slot_size - 20
    ...
    def _draw_slot(self, screen, x, y, item, slot_id):
        ...
        if item:
            # Icono (texto por ahora, puedes usar imágenes)
            ...
            # Cantidad (si es más de 1)
            if item.cantidad > 1:
                font_cant = pygame.font.SysFont("Arial", 14, bold=True)
                cant_text = font_cant.render(str(item.cantidad), True, self.color_cantidad)
                
                # Sombra del texto
                sombra = font_cant.render(str(item.cantidad), True, (0, 0, 0))
                screen.blit(sombra, (x + self.slot_size - 20, y + self.slot_size - 18))
