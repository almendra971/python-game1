...
from inventario import Inventory  # <-- NUEVO

pygame.init()

ventana = pygame.display.set_mode((texturas.width, texturas.height))
pygame.display.set_caption("Mi Juego con Inventario Terraria")
...
def main():
    ...
    while True:
            ...
            if mostrar_mundo:
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_e:
                        ...
                        for roca in mundo.rocas[:]:  
                            
                            dist_x = abs(personaje.x - roca.x)
                            dist_y = abs(personaje.y - roca.y)
                            
                            
                            if dist_x < 40 and dist_y < 40:
                     
                                if inventario.agregar_item("Roca", 1):
                                    mundo.rocas.remove(roca)
                                    print("✓ Recogido: Roca")
                                else:
        ...
        if mostrar_dialogos:
            ...
            instrucciones = [
                "E: Recoger objetos | I: Inventario",
                "1-0: Hotbar | Mouse: Arrastrar items"
            ]
            y_offset = 10
            for texto in instrucciones:
                texto_render = font_instruc.render(texto, True, (150, 255, 150))
                ventana.blit(texto_render, (10, y_offset))
                y_offset += 20
